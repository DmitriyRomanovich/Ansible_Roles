...
JAVA17_VERSION = "17.0.3+7"
JAVA17_HOME = "/opt/java/jdk-{}".format(JAVA17_VERSION)
...
def test_java_version(host):
    ...
    elif host.ansible.get_variables()["inventory_hostname"] == "ics-ans-role-java-openjdk17":
        assert 'build {}'.format(JAVA17_VERSION) in cmd.stdout
    else:
        assert 'build {}'.format(JAVA_VERSION) in cmd.stdout


def test_java17_version(host):
    cmd = host.run("java -version 2>&1")
    if host.ansible.get_variables()["inventory_hostname"] == "ics-ans-role-java-openjdk17":
        assert 'build {}'.format(JAVA17_VERSION) in cmd.stdout
...
def test_secure_random_source(host):
    if host.ansible.get_variables()["inventory_hostname"] == "ics-ans-role-java-and-oracle-jdk8":
        ...
        java_security = host.file("{}/conf/security/java.security".format(JAVA17_HOME))
    else:
